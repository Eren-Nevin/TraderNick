...
# V4 schema is flatter: every event row carries a single `user` column
# (= the position-holder). Liquidations also use `user` (the liquidated
# party) — there's no separate `owner` field. So all 5 legs key on `user`.
_LEADERBOARD_EVENTS_V4 = [
    ("tradernick.aave_v4_deposits",     "token",      "deposit"),
    ("tradernick.aave_v4_withdrawals",  "token",      "withdraw"),
    ("tradernick.aave_v4_borrows",      "token",      "borrow"),
    ("tradernick.aave_v4_repays",       "token",      "repay"),
    ("tradernick.aave_v4_liquidations", "debt_token", "liquidation"),
]
...
@bp.get("/aave_v4/wallets/leaderboard")
async def leaderboard(request):
    ...
    legs = []
    for table, token_col, ev_label in _LEADERBOARD_EVENTS_V4:
        pairs = [(c, t) for c in chains for t in tokens]
        tup = ", ".join(f"({_qlit(c)}, {_qlit(t)})" for (c, t) in pairs)
        ckt = f"(chain, {token_col}) IN ({tup})"
        legs.append(f"""
            SELECT user AS wallet, coalesce(value_usd, 0) AS value_usd, '{ev_label}' AS ev
            FROM {table} FINAL
            WHERE {ckt}
              AND time >= {{since:DateTime}}
              AND time <  {{until:DateTime}}
