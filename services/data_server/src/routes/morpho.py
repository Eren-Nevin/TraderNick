"""Morpho events aggregate endpoint. 7 events. Charts select by (chain,
token); the per-row market_id is preserved in CH but not exposed as a
selector dimension (markets are summed across)."""
...
# Event → (table, amount-expression, token-column-override).
# Lending events use `assets` (token amount). Collateral events same.
# Liquidation uses repaid_assets as headline; token column is loan_token.
_EVENT_TABLES = {
    "supply":              ("tradernick.morpho_supplies",              "assets",        None),
    "withdraw":            ("tradernick.morpho_withdrawals",           "assets",        None),
    "borrow":              ("tradernick.morpho_borrows",               "assets",        None),
    "repay":               ("tradernick.morpho_repays",                "assets",        None),
    "supply_collateral":   ("tradernick.morpho_supply_collaterals",    "assets",        None),
    "withdraw_collateral": ("tradernick.morpho_withdraw_collaterals",  "assets",        None),
    "liquidation":         ("tradernick.morpho_liquidations",          "repaid_assets", "loan_token"),
}
