"""Binance funding_rate backfill. defistream 2.22 multi-token form."""
import asyncio
import json
import logging
import signal
...
import config
from clickhouse import FUNDING_RATE_COLUMNS, async_client, force_purge_tokens, funding_rate_df_to_rows

logging.basicConfig(level=logging.INFO, format="%(asctime)s [backfill_funding_rate] %(levelname)s %(message)s")
log = logging.getLogger(__name__)

CHUNK_DAYS = 31
TABLE = "tradernick.binance_funding_rate"
_stop = False
...
async def _fetch_chunk(ds: AsyncDeFiStream, tokens: list[str], since: datetime, until: datetime) -> int:
    ...
    rows = funding_rate_df_to_rows(df)
    ch = await async_client()
    await ch.insert(TABLE, rows, column_names=FUNDING_RATE_COLUMNS)
    return len(rows)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_binance_funding_rate <job_id>", file=sys.stderr); sys.exit(2)
