...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [backfill_tron_native_transfers] %(levelname)s %(message)s")
...
async def _fetch_chunk(ds: AsyncDeFiStream, since: datetime, until: datetime) -> int:
    ...
    if df.is_empty():
        return 0
    pd_df = transfers_df_for_bulk_insert(df, kind="tron_native", chain="TRON", token_override="TRX")
    ch = await async_client()
...
async def main(job_id: str):
    ...
    if args.get("force") and done == 0:
        log.info("job %s force=true: purging existing tron_native rows in [%s, %s)", job_id, since, until)
        await delete_transfers_range(
            where_extra="kind = 'tron_native' AND chain = 'TRON'",
            since=since,
            until=until,
        )
    ...
    try:
        for cs, ce in chunks:
            ...
            if key in completed_set:
                continue
            log.info("job %s chunk TRX %s..%s", job_id, cs, ce)
            n = await _fetch_chunk(ds, cs, ce)
            log.info("job %s chunk TRX rows=%d", job_id, n)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_tron_native_transfers <job_id>", file=sys.stderr)
        sys.exit(2)
