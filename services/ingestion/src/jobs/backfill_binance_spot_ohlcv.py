"""Binance SPOT OHLCV backfill. Mirror of backfill_binance_ohlcv (perp/futures)
— the only difference is `.market("spot")` on the SDK call and the destination
table. One multi-token call per 7-day chunk."""
import asyncio
import json
import logging
import signal
...
import config
from clickhouse import OHLCV_COLUMNS, async_client, force_purge_tokens, ohlcv_df_to_rows
...
async def main(job_id: str):
    ...
    # Token-scoped force purge — only on a fresh run (done==0), never on resume.
    if args.get("force") and done == 0:
        where = await force_purge_tokens(
            table="tradernick.binance_spot_ohlcv_1m", tokens=tokens, since=since, until=until)
        log.info("job %s force purge: tradernick.binance_spot_ohlcv_1m WHERE %s", job_id, where)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_binance_spot_ohlcv <job_id>", file=sys.stderr)
        sys.exit(2)
