...
def _planned_chunks(chains: list[str], since: datetime, until: datetime):
    chunks = []
    ...
    for chain in chains:
        ...
        while t < until:
            ...
            chunks.append((chain, t, t_end))
            t = t_end
    return chunks


async def _fetch_chunk(ds: AsyncDeFiStream, chain: str, since: datetime, until: datetime) -> int:
    df = await (
...
async def main(job_id: str):
    ...
    chunks = _planned_chunks(chains, since, until)
    total = len(chunks)
    done = sum(1 for chain, cs, _ in chunks if (chain, cs.isoformat()) in completed_set)
    ...
    try:
        for chain, cs, ce in chunks:
            ...
            if key in completed_set:
                continue
            log.info("job %s chunk %s %s..%s", job_id, chain, cs, ce)
            n = await _fetch_chunk(ds, chain, cs, ce)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_evm_native_transfers <job_id>", file=sys.stderr)
        sys.exit(2)
