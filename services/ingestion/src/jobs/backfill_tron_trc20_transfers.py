...
def _planned_chunks(tokens: list[str], since: datetime, until: datetime):
    chunks = []
...
async def _fetch_chunk(ds: AsyncDeFiStream, token: str, since: datetime, until: datetime) -> int:
    ...
    if df.is_empty():
        return 0
    pd_df = transfers_df_for_bulk_insert(df, kind="trc20", chain="TRON", token_override=token)
    ch = await async_client()
...
async def main(job_id: str):
    ...
    log.info("job %s starting: tokens=%s chunks=%d resumed_at=%d force=%s",
             job_id, tokens, total, done, bool(args.get("force")))

    if args.get("force") and tokens and done == 0:
        token_clauses = " OR ".join(f"token = '{safe_ident(t)}'" for t in tokens)
        log.info("job %s force=true: purging existing trc20 rows for tokens=%s in [%s, %s)",
                 job_id, tokens, since, until)
        await delete_transfers_range(
            where_extra=f"kind = 'trc20' AND chain = 'TRON' AND ({token_clauses})",
            since=since,
            until=until,
        )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_tron_trc20_transfers <job_id>", file=sys.stderr)
        sys.exit(2)
