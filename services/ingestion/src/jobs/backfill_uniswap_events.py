"""Uniswap V3 events backfill — mirrors backfill_aave_events.py.

Iterates chunks of (chain, symbol0, symbol1, fee, event, start, end) through
DeFiStream and inserts into the matching per-event Uniswap table. Resumable
via completed_chunks list in the job args, 429-aware retry with backoff,
1.2s sleep between chunks to stay under the rate limit.

Job body:
  {
    "days": 30,
    "pools": [["ETH","USDC","WETH",500], ...],          # optional, defaults to config.UNI_V3_POOLS
    "events": ["swap","deposit","withdraw","collect"],  # optional, all by default
...
import config
from clickhouse import UNISWAP_EVENTS, async_client, safe_ident

logging.basicConfig(level=logging.INFO, format="%(asctime)s [backfill_uniswap_events] %(levelname)s %(message)s")
    ...
    for chain, sym0, sym1, fee in pools:
        for event in events:
            ...
            while t < until:
                ...
                chunks.append((chain, sym0, sym1, int(fee), event, t, t_end))
                t = t_end
    return chunks
...
async def main(job_id: str):
    ...
    unknown = [e for e in events if e not in UNISWAP_EVENTS]
    if unknown:
    ...
    total = len(chunks)
        ...
        1 for chain, sym0, sym1, fee, event, cs, _ in chunks
        if (chain, sym0, sym1, fee, event, cs.isoformat()) in completed_set
    )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_uniswap_events <job_id>", file=sys.stderr)
        sys.exit(2)
