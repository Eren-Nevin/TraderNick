"""AAVE v2 events backfill — same shape as backfill_aave_events.py minus
the eth_market axis. Chunks 24h per (chain, event), 1.2s pacing, 429
retry with backoff, and skips chains DeFiStream isn't configured for.
"""
import asyncio
import json
import logging
...
import config
from clickhouse import AAVE_V2_EVENTS, async_client, safe_ident

logging.basicConfig(level=logging.INFO, format="%(asctime)s [backfill_aave_v2_events] %(levelname)s %(message)s")
    ...
    for chain in chains:
        for event in events:
            ...
            while t < until:
                ...
                out.append((chain, event, t, t_end))
                t = t_end
    return out


async def _fetch_chunk(ds, *, chain, event, since, until) -> int:
...
async def main(job_id: str):
    ...
    chains = args.get("chains") or list(config.AAVE_V2_CHAINS)
    events = args.get("events") or list(AAVE_V2_EVENTS.keys())
    unknown = [e for e in events if e not in AAVE_V2_EVENTS]
    if unknown:
    ...
    log.info("job %s starting: pairs=%d chunks=%d resumed_at=%d",
             job_id, len(chains) * len(events), total, done)

    if args.get("force") and done == 0:
        ch = await async_client()
        for event in events:
            _, table, _, _ = AAVE_V2_EVENTS[event]
            for chain in chains:
                where = (
    ...
    try:
        for chain, event, cs, ce in chunks:
            ...
            try:
                n = await _fetch_chunk(ds, chain=chain, event=event, since=cs, until=ce)
            except Exception as exc:
                if _is_not_supported(exc):
                    log.warning("pair %s not on DeFiStream — skipping: %s", label, exc)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_aave_v2_events <job_id>", file=sys.stderr)
        sys.exit(2)
