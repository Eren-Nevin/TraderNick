"""Aerodrome basic-pool events backfill (BASE only). Pool identity:
(chain, sym0, sym1, stable). Same 24h/1.2s/dead-skip pattern."""
import asyncio, json, logging, signal, sys
from datetime import datetime, timedelta, timezone

from defistream import AsyncDeFiStream
import config
from clickhouse import AERO_BASIC_EVENTS, async_client, safe_ident
...
def _is_not_supported(exc):
    m = str(exc).lower()
    return ("pool not found" in m or "pair not found" in m
            or "not available" in m or "not supported" in m or "not configured" in m)


def _planned_chunks(pools, events, since, until):
    out = []
    step = timedelta(hours=CHUNK_HOURS)
    for (c, s0, s1, st) in pools:
        for ev in events:
            t = since
            while t < until:
                t_end = min(t + step, until)
                out.append((c, s0, s1, st, ev, t, t_end))
                t = t_end
    return out


async def _fetch_chunk(ds, *, chain, symbol0, symbol1, stable, event, since, until):
...
async def main(job_id):
    ...
    # Mirror live worker pool selection: AERO_BASIC_LIVE_POOLS or AERO_BASIC_POOLS.
    live_default = config.AERO_BASIC_LIVE_POOLS or config.AERO_BASIC_POOLS
    pools_arg = args.get("pools") or [[c,s0,s1,st] for (c,s0,s1,st) in live_default]
    pools = [(p[0],p[1],p[2],bool(p[3])) for p in pools_arg]
    events = args.get("events") or list(AERO_BASIC_EVENTS.keys())
    unknown = [e for e in events if e not in AERO_BASIC_EVENTS]
    ...
    total = len(chunks)
    done = sum(1 for (c,s0,s1,st,ev,cs,_) in chunks if (c,s0,s1,st,ev,cs.isoformat()) in completed_set)
    ...
    log.info("job %s starting: pools=%d chunks=%d resumed_at=%d", job_id, len(pools), total, done)

    if args.get("force") and done == 0:
        ch = await async_client()
        for ev in events:
            _, table, _, _ = AERO_BASIC_EVENTS[ev]
            for (c,s0,s1,st) in pools:
                where = (f"chain = '{safe_ident(c)}' AND symbol0 = '{safe_ident(s0)}'"
                         f" AND symbol1 = '{safe_ident(s1)}' AND stable = {1 if st else 0}"
    ...
    dead_pools = set()
    try:
        for (c,s0,s1,st,ev,cs,ce) in chunks:
            if _stop:
                ...
                return
            key = (c,s0,s1,st,ev,cs.isoformat())
            if key in completed_set: continue
            pool_key = (c,s0,s1,st)
            label = f"{c}/{s0}-{s1}/{'s' if st else 'v'}/{ev}"
            if pool_key in dead_pools:
                completed_set.add(key); done += 1; continue
            log.info("chunk %s %s..%s", label, cs, ce)
            try:
                n = await _fetch_chunk(ds, chain=c, symbol0=s0, symbol1=s1,
                                       stable=st, event=ev, since=cs, until=ce)
            except Exception as exc:
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_aero_basic_events <job_id>", file=sys.stderr); sys.exit(2)
