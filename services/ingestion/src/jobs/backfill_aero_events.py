"""Aerodrome concentrated-pool events backfill (BASE only).

Pool keyed by (chain=BASE, sym0, sym1, tick_spacing). Same 24h-chunk +
1.2s pacing + dead-pool skip pattern as the V3/V4 drivers."""
import asyncio, json, logging, signal, sys
from datetime import datetime, timedelta, timezone

from defistream import AsyncDeFiStream
import config
from clickhouse import AERO_CL_EVENTS, async_client, safe_ident

logging.basicConfig(level=logging.INFO, format="%(asctime)s [backfill_aero_events] %(levelname)s %(message)s")
...
def _planned_chunks(pools, events, since, until):
    ...
    for (c, s0, s1, ts) in pools:
        for ev in events:
            t = since
            while t < until:
                t_end = min(t + step, until)
                out.append((c, s0, s1, ts, ev, t, t_end))
                t = t_end
    return out


async def _fetch_chunk(ds, *, chain, symbol0, symbol1, tick_spacing, event, since, until):
...
async def main(job_id):
    ...
    total = len(chunks)
    done = sum(1 for (c,s0,s1,ts,ev,cs,_) in chunks if (c,s0,s1,ts,ev,cs.isoformat()) in completed_set)
    ...
    if args.get("force") and done == 0:
        ch = await async_client()
        for ev in events:
            _, table, _, _ = AERO_CL_EVENTS[ev]
            for (c,s0,s1,ts) in pools:
                where = (f"chain = '{safe_ident(c)}' AND symbol0 = '{safe_ident(s0)}'"
                         f" AND symbol1 = '{safe_ident(s1)}' AND tick_spacing = {int(ts)}"
    ...
    dead_pools = set()
    try:
        for (c,s0,s1,ts,ev,cs,ce) in chunks:
            if _stop:
                ...
                return
            key = (c,s0,s1,ts,ev,cs.isoformat())
            if key in completed_set: continue
            pool_key = (c,s0,s1,ts)
            label = f"{c}/{s0}-{s1}/{ts}/{ev}"
            ...
            try:
                n = await _fetch_chunk(ds, chain=c, symbol0=s0, symbol1=s1,
                                       tick_spacing=ts, event=ev, since=cs, until=ce)
            except Exception as exc:
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_aero_events <job_id>", file=sys.stderr); sys.exit(2)
