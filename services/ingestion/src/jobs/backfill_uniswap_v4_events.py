"""Uniswap V4 events backfill — pool keyed by (chain, sym0, sym1, fee,
tick_spacing, hooks). Same 24h-chunk + 1.2s pacing + dead-pool skip as
the V3 driver."""
...
import config
from clickhouse import UNISWAP_V4_EVENTS, async_client, safe_ident

logging.basicConfig(level=logging.INFO, format="%(asctime)s [backfill_uniswap_v4_events] %(levelname)s %(message)s")
    ...
    for (c, s0, s1, fee, ts, hk) in pools:
        for ev in events:
            ...
            while t < until:
                ...
                out.append((c, s0, s1, fee, ts, hk, ev, t, t_end))
                t = t_end
    return out


async def _fetch_chunk(ds, *, chain, symbol0, symbol1, fee, tick_spacing, hooks, event, since, until):
...
async def main(job_id):
    ...
    total = len(chunks)
    done = sum(1 for (c,s0,s1,fee,ts,hk,ev,cs,_) in chunks if (c,s0,s1,fee,ts,hk,ev,cs.isoformat()) in completed_set)
    ...
    if args.get("force") and done == 0:
        ch = await async_client()
        for ev in events:
            _, table, _, _ = UNISWAP_V4_EVENTS[ev]
            for (c,s0,s1,fee,ts,hk) in pools:
    ...
    dead_pools = set()
    try:
        for (c,s0,s1,fee,ts,hk,ev,cs,ce) in chunks:
            if _stop:
                ...
                return
            key = (c,s0,s1,fee,ts,hk,ev,cs.isoformat())
            if key in completed_set: continue
            pool_key = (c,s0,s1,fee,ts,hk)
            label = f"{c}/{s0}-{s1}/{fee}/{ts}/{ev}"
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_uniswap_v4_events <job_id>", file=sys.stderr); sys.exit(2)
