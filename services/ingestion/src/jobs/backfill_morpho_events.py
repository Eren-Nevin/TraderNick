"""Morpho events backfill (ETH + BASE). 7 events × chains. Same 24h/1.2s/
...
import config
from clickhouse import MORPHO_EVENTS, async_client, safe_ident

logging.basicConfig(level=logging.INFO, format="%(asctime)s [backfill_morpho_events] %(levelname)s %(message)s")
...
async def main(job_id):
    ...
    chains = args.get("chains") or list(config.MORPHO_CHAINS)
    events = args.get("events") or list(MORPHO_EVENTS.keys())
    unknown = [e for e in events if e not in MORPHO_EVENTS]
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_morpho_events <job_id>", file=sys.stderr); sys.exit(2)
