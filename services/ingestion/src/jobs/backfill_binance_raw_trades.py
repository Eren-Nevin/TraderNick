...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [backfill_raw_trades] %(levelname)s %(message)s")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_binance_raw_trades <job_id>", file=sys.stderr)
        sys.exit(2)
