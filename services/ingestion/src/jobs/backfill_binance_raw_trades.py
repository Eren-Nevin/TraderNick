"""Binance raw_trades backfill. defistream 2.22 multi-token form — one
call per chunk covers every configured token. Chunks are kept tight
(6h) so the per-call row count stays bounded even when summing 23
tokens' trade volume together."""
import asyncio
import json
import logging
import signal
...
import config
from clickhouse import async_client, force_purge_tokens, raw_trades_df_for_insert
...
async def main(job_id: str):
    ...
    if args.get("force") and done == 0:
        where = await force_purge_tokens(
            table="tradernick.binance_raw_trades", tokens=tokens, since=since, until=until)
        log.info("job %s force purge: tradernick.binance_raw_trades WHERE %s", job_id, where)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_binance_raw_trades <job_id>", file=sys.stderr); sys.exit(2)
