"""Binance SPOT raw_trades backfill. Mirror of backfill_binance_raw_trades
(perp/futures) — the only difference is `.market("spot")` on the SDK call and
the destination table. One multi-token call per 6h chunk."""
import asyncio
import json
import logging
import signal
...
async def main(job_id: str):
    ...
    if args.get("force") and done == 0:
        where = await force_purge_tokens(
            table="tradernick.binance_raw_spot_trades", tokens=tokens, since=since, until=until)
        log.info("job %s force purge: tradernick.binance_raw_spot_trades WHERE %s", job_id, where)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_binance_spot_raw_trades <job_id>", file=sys.stderr); sys.exit(2)
