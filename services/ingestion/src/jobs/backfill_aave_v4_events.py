"""AAVE v4 events backfill (ETH only). 5 events × chains. Same 24h-chunk
+ 1.2s pacing + dead-chain skip pattern as the V2/V3 drivers."""
import asyncio, json, logging, signal, sys
from datetime import datetime, timedelta, timezone

from defistream import AsyncDeFiStream
import config
from clickhouse import AAVE_V4_EVENTS, async_client, safe_ident

logging.basicConfig(level=logging.INFO, format="%(asctime)s [backfill_aave_v4_events] %(levelname)s %(message)s")
...
async def main(job_id):
    ...
    chains = args.get("chains") or list(config.AAVE_V4_CHAINS)
    events = args.get("events") or list(AAVE_V4_EVENTS.keys())
    unknown = [e for e in events if e not in AAVE_V4_EVENTS]
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("usage: python -m jobs.backfill_aave_v4_events <job_id>", file=sys.stderr); sys.exit(2)
