...
async def main(stream_name: str | None = None):
    ...
    async def sweep_loop(once: bool = False):
        ...
        while True:
            ...
            _sweep_t0 = time.monotonic()
            try:
                last_seen = await min_watermark_per_token(ch, table="tradernick.binance_funding_rate", tokens=tokens, max_staleness_seconds=30 * 24 * 3600)
                ...
                log.exception("binance_funding_rate sweep failed: %s", exc)
