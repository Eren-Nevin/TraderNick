"""Live polling for Binance perp funding rate. defistream 2.22 multi-token
...
async def main():
    ...
    async def gap_fill_task():
        ...
        last_seen = await min_watermark_per_token(
            ch, table="tradernick.binance_funding_rate", tokens=tokens,
        )
        since = resolve_since(last_seen, t_start=t_start)
        if since >= t_start:
            return
        log.info("binance_funding_rate gap-fill since=%s until=%s (min_last_seen=%s, tokens=%d)",
                 since, t_start, last_seen, len(tokens))
        async def call(s, u):
            return await fetch_and_insert(ds, tokens, s, u)
        total = await run_chunked(label="binance_funding_rate", since=since, until=t_start, call=call)
        log.info("binance_funding_rate gap-fill done total_rows=%d", total)
