...
async def main():
    ...
    log.info("polling open_interest tokens=%s every %ss (overlap=%dm) + gap-fill from watermark",
    ...
    async def gap_fill_task():
        ...
        async def _one(token):
            last_seen = await latest_time(
                ch, table="tradernick.binance_open_interest",
                where="token = {token:String}",
                parameters={"token": token},
            )
            since = resolve_since(last_seen, t_start=t_start)
            if since >= t_start: return
            label = f"binance_open_interest/{token}"
