"""Live polling for Binance SPOT 1m OHLCV. Mirror of groups/binance_ohlcv
...
Gap-fill takes the MIN(MAX(time)) across all tokens as the start, then fires one
multi-token call to recover [min_last_seen - overlap, t_start]. Tokens with more
recent data get over-fetched but ReplacingMergeTree dedupes on (token, time)."""
import asyncio
import logging
import sys
import time
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [binance_spot_ohlcv] %(levelname)s %(message)s")
...
async def fetch_and_insert(ds: AsyncDeFiStream, tokens: list[str], since: datetime, until: datetime) -> int:
    ...
    await ch.insert("tradernick.binance_spot_ohlcv_1m", rows, column_names=OHLCV_COLUMNS)
    return len(rows)
...
async def main(stream_name: str | None = None):
    ...
    async def live_loop():
        ...
        while True:
            ...
            try:
                ...
                log.info("multi-token spot rows=%d (tokens=%d)", n, len(tokens))
    ...
    async def sweep_loop(once: bool = False):
        ...
        while True:
            ...
            try:
                last_seen = await min_watermark_per_token(ch, table="tradernick.binance_spot_ohlcv_1m", tokens=tokens, max_staleness_seconds=30 * 24 * 3600)
                since = sweep.sweep_since(
                    now=now,
                    sweep_cadence_seconds=sweep_cadence,
                    last_seen=last_seen,
                    # DeFiStream OHLCV cap is 31 days; leave 1 day of slack.
                    max_window_seconds=30 * 24 * 3600,
                    stream_name=stream_name or "binance_spot_ohlcv",
                )
                if since < now:
                    n = await fetch_and_insert(ds, tokens, since, now)
                    log.info("binance_spot_ohlcv sweep window=%s..%s rows=%d (min_last_seen=%s, tokens=%d)", since, now, n, last_seen, len(tokens))
            except Exception as exc:
                log.exception("binance_spot_ohlcv sweep failed: %s", exc)
