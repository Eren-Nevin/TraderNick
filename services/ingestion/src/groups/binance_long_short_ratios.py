...
async def main(stream_name: str | None = None):
    ...
    async def sweep_loop(once: bool = False):
        ...
        while True:
            ...
            _sweep_t0 = time.monotonic()
            try:
                last_seen = await min_watermark_per_token(ch, table="tradernick.binance_long_short_ratios", tokens=tokens, max_staleness_seconds=30 * 24 * 3600)
                since = sweep.sweep_since(
                    now=now,
                    sweep_cadence_seconds=sweep_cadence,
                    last_seen=last_seen,
                    # DeFiStream LSR cap is 31 days; leave 1 day of slack.
                    max_window_seconds=30 * 24 * 3600,
                    stream_name=stream_name or "binance_long_short_ratios",
                )
                if since < now:
                    n = await fetch_and_insert(ds, tokens, since, now)
                    log.info("binance_long_short_ratios sweep window=%s..%s rows=%d (min_last_seen=%s, tokens=%d)", since, now, n, last_seen, len(tokens))
            except Exception as exc:
                log.exception("binance_long_short_ratios sweep failed: %s", exc)
