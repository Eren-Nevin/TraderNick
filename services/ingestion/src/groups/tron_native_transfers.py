...
async def main():
    ...
    log.info("polling tron native transfers every %ss (overlap=%dm) + gap-fill from watermark",
    ...
    async def gap_fill_task():
        ...
        log.info("tron_native_transfers gap-fill since=%s until=%s (last_seen=%s)", since, t_start, last_seen)
        async def call(s, u):
            return await fetch_and_insert(ds, s, u)
        total = await run_chunked(label="tron_native_transfers", since=since, until=t_start, call=call)
        log.info("tron_native_transfers gap-fill done total_rows=%d", total)
