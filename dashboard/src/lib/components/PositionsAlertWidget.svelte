<script lang="ts">
  ...
  const selCls =
    'rounded border border-zinc-700 bg-zinc-950 px-2 py-1 text-sm text-zinc-100 focus:border-zinc-500 focus:outline-none';
</script>
...
<div class="flex h-full flex-col gap-2 p-3 text-sm text-zinc-200" use:stopDragEvents>
  ...
  <div class="flex items-center gap-2">
    ...
    <input
      class="min-w-0 flex-1 rounded border border-transparent bg-transparent px-1 py-0.5 text-base font-semibold text-zinc-100 hover:border-zinc-700 focus:border-zinc-600 focus:bg-zinc-950 focus:outline-none"
  ...
  <div class="min-h-0 flex-1 overflow-y-auto">
    <div class="grid grid-cols-2 gap-x-3 gap-y-2">
      <label class="col-span-2 text-xs text-zinc-400">
        Wallet group
        <select class="{selCls} mt-1 block w-full"
      ...
      <label class="text-xs text-zinc-400">
        Top N (per side)
        <select class="{selCls} mt-1 block w-full" bind:value={instance.paTopN}>
      ...
      <label class="text-xs text-zinc-400">
        Position staleness
        <select class="{selCls} mt-1 block w-full" bind:value={instance.paStaleness}>
      ...
      <label class="text-xs text-zinc-400">
        Report every
        <select class="{selCls} mt-1 block w-full" bind:value={instance.paCadence}>
          {#each CADENCE as c (c)}<option value={c}>{c}</option>{/each}
        </select>
      </label>
    </div>
    <p class="mt-2 text-[11px] text-zinc-600">
      Reports the top {instance.paTopN ?? '5'} most-long and most-short tokens by
      {instance.paCriteria === 'net_size' ? 'net size' : 'net long'}, every {instance.paCadence ?? '5m'}.
    </p>
  </div>
